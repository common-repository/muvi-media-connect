...
registerBlockType( 'muvi-media-connect/block-muvi-media-connect', {
	...
	edit: ( { attributes, setAttributes } ) => {
		...
		setAttributes( { muviLiveApiUrl: cgbGlobal.muvi_flex_api_url } );
		...
		// setAttributes( { selectedItemCounts: 0 } );
		const [ isOpen, setOpen ] = useState( false );
		const openModal = () =>{
			setAttributes( { selectedArray: [] } );
			setOpen( true );
			if ( attributes.muviProductSelected === 'muvi 6' ) {
				console.log("Category listing is going here....!");
				console.log(attributes.categoryList[ 0 ].category_name);
				console.log(attributes.categoryList[ 0 ].category_uuid);
				if( attributes.categoryList[ 0 ].category_name === 'Live' ){

					setAttributes( { muviCategoryUuid: attributes.categoryList[ 0 ].category_uuid } );
					onSelectMuviLive( attributes.categoryList[ 0 ].category_name );
		...
		const onSelectMuviLive = async (tabName) => {
			try {
				setAttributes({ muviLoader: 0 });
				const { categoryList, muviBearerToken, muviLiveApiUrl, appId } = attributes;
				...
				const contentQuery = {
					query: '{ contentList(caller:\"CMS\",product_key:\":product_key\",app_token:\":app_token\",extension:\"live\",content_name:\"\",sort_by:\"nto\",page:0,per_page:10){ page_info{total_count} content_list{content_uuid content_name content_desc content_created_date content_permalink extension custom_field_data { custom_field_value custom_field_name }} stream_list {livestream_uuid stream_type content_uuid server_state status app_token product_key m5ext stream_key}}}',
				};
				...
				const response = await axios.post(`${muviLiveApiUrl}/livestream`, contentQuery, { headers: embdHeaders });
				...
				// Fetch embed URLs for each content
				await Promise.all(contentList.stream_list.map(async (stream) => {
					try {
						const embedResponse = await axios.post(
							`${muviLiveApiUrl}/stream/get-embed-url`,
							{ stream_key: stream.stream_key },
							{ headers: embdHeaders }
						);
						const embedUrl = embedResponse.data.data.embed_url;
		
						// Find the matching content_uuid in content_list and add the embed_url
						const contentItem = contentList.content_list.find(item => item.content_uuid === stream.content_uuid);
						if (contentItem) {
							contentItem.embed_url = embedUrl;
						}
					} catch (error) {
		...
		const videoSelected = () => {
			...
			setAttributes( { selectedProp: [] } );
			setAttributes( { selectedArray: [] } );

			closeModal();
		};
		const selectOnClick = ( props , arg2 ) => {
			
			let selectedValue;

			if(arg2 == 1){
				selectedValue = '[muvimedia permalink="' + props.target.value + '" height="' + attributes.muviHeight + '" width="' + attributes.muviWidth + '"]';
			}else{
				selectedValue = '[muvimedia permalink="' + props.target.value + '" height="' + attributes.muviAudioHeight + '" width="' + attributes.muviAudioWidth + '"]';					
			}

			if (props.target.checked) {
				// If checkbox is checked, add the value to the array
				attributes.selectedArray.push(selectedValue);
			} else {
				// If checkbox is unchecked, remove the value from the array
				const index = attributes.selectedArray.indexOf(selectedValue);
				if (index !== -1) {
					attributes.selectedArray.splice(index, 1);
				}
			}

			// Copy selectedArray to selectedProp
			attributes.selectedProp = [...attributes.selectedArray];

			if(attributes.selectedProp.length > 0){
				setAttributes( { selectStatus: 1 } );
			}else{
				setAttributes( { selectStatus: 0 } );
			}
		...
		const selectOnLiveClick = ( props , arg2 , indexed ) => {
			...
			selectedValue = '[muvimedialive permalink="' + props.target.value + '" height="' + attributes.muviHeight + '" width="' + attributes.muviWidth + '"]';
		...
		const selectOnFlexClick = ( props , arg2 , indexed ) => {
			...
			if (attributes.muviProductSelected === 'muvi 6') {
				if(arg2 == 1){
					selectedValue = '[muvimediaflex permalink="' + props.target.value + '" height="' + attributes.muviHeight + '" width="' + attributes.muviWidth + '"]';
				}else{
					selectedValue = '[muvimediaflex permalink="' + props.target.value + '" height="' + attributes.muviAudioHeight + '" width="' + attributes.muviAudioWidth + '"]';					
				}
			}else{
				selectedValue = '[muvimedia permalink="' + props.target.value + '" height="' + attributes.muviHeight + '" width="' + attributes.muviWidth + '"]';
			}
		...
		return (
			...
			{ attributes.shortCode !== undefined ? ( <div>
					...
					<div className="fullMediaConnect" style={{ margin: '10px' }}>
							...
							<div className="topTitle">
								...
								Muvi Media Connect Shortcode
							</div>
							{myShortcode.map((shortcode, index) => (
								<div key={index}>
									<textarea
										className="block-editor-plain-text blocks-shortcode__textarea updateCls"
										rows={2}
										value={shortcode}
									/>
								</div>
																...
																<div>
																	{ attributes.muviLoader === 0 ? <div className="spinner-border" role="status">
																		...
																		{ attributes.muviProductSelected === 'muvi 5' ?
																						...
																						<li key={ indexed } data-permalink={ attributes.movie[ indexed ].permalink } className={ attributes.movie[ indexed ].permalink === attributes.selectedProp ? attributes.cssClassReplace : null } data-hasepisodes="0">
																							<div className="planSingleBox">
																								<div className="select-package-checkbox">
																									<input type="checkbox" name={`selectpackage_${indexed}`} value={ attributes.movie[ indexed ].permalink } onClick={(e) => selectOnClick(e, attributes.movie[indexed].content_types_id) } id={ indexed } />
																			...
																			</ul> : attributes.categoryList[ 0 ].category_name === 'Live' ?
																			<ul className="muvi_content_list">
																				{ ln > 0 ? obj.map((muvilist, indexed) => (
																					<li key={indexed}>
																						<div className="planSingleBox">
																							<div className="select-package-checkbox">
																								<input type="checkbox" name={`selectpackage_${indexed}`} value={ attributes.movie[ indexed ].embed_url } onClick={(e) => selectOnLiveClick(e, attributes.movie[indexed].content_uuid, indexed)} id={ indexed } />
																								<label htmlFor={indexed}>
																									<div className="item">
																										<div className="inner-item">
																											<img src='https://liveservercms.muvi.com/_assets/images/grayscale-mountain.jpg' />
																										</div>
																				...
																				{ ln > 0 ? obj.map( ( muvilist, indexed ) => {
																					return (
																						<li key={ indexed } data-permalink={ attributes.movie[ indexed ].permalink } className={ attributes.movie[ indexed ].permalink === attributes.selectedProp ? attributes.cssClassReplace : null } data-hasepisodes="0">
																							<div className="planSingleBox">
																								<div className="select-package-checkbox">
																									<input type="checkbox" name={`selectpackage_${indexed}`} value={ attributes.movie[ indexed ].embd } onClick={(e) => selectOnFlexClick(e, attributes.movie[indexed].content_asset_type, indexed)} id={ indexed } />
																									<label htmlFor={ indexed }>
																										<div className="item">
																											<div className="inner-item">
	 ...
	 */
	save: ({ attributes }) => {
		const myShortcode = attributes.shortCode ? attributes.shortCode : [];
